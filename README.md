...
轻量级的个人在线书签管理工具，简单易用和高效部署，项目源代码不到 200KB，仅依赖 PHP 和 MySQL，无需复杂环境即可在虚拟主机上运行，将零散的浏览器书签网址集中式管理，做到一处部署，任意一台联网设备上都能访问自己的书签。
...
2. Chrome 浏览器插件（可选）：
   ...
   修改插件文件夹 `bookmark-extension` 内 3 个文件
...
bookmarks/
  ...
  ├── add_memo.php             # 添加备忘录
  ├── edit_memo.php            # 编辑备忘录
  ├── delete_memo.php          # 删除备忘录
  ├── generate_share_token.php # 备忘录 AJAX 生成分享 token
  ├── share_memo.php           # 备忘录公开分享页面（无需登录）
  ├── footer.php               # 页脚
  ├── images/                  # 页面展示
  │   ├── content1.png
  │   └── content2.png
...
4. 设置：
   - 在顶部导航栏点击“设置”图标，进入设置页可选择默认书签视图、修改密码、导出导入书签、生成API密钥（用于浏览器插件）、关闭或开启注册（管理员）、页脚设置（管理员）。
5. 备忘录：
   - 在顶部导航栏点击“备忘录”图标，进入备忘录页可添加、编辑、删除、复制、分享。
6. 搜索：
   - 在搜索框输入书签标题或者书签链接，按 Enter 或点击搜索。可以在搜索框左侧下拉菜单中切换搜索引擎，选择本站则搜索书签，选择备忘录则搜索备忘录，选择百度/谷歌则在新标签页中打开搜索结果。
7. 书签排序：
